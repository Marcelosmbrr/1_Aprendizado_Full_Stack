...
function carregar() {
  //DEFAULT
  document.body.style.backgroundColor = "#333"; //HEADER
  ...
  header.style.backgroundColor = "whitesmoke"; //IMG PARA O LOGO
  ...
  imagem_logo.style.borderRadius = "0px 30px 30px 0px"; //CONTEÚDOS DO HEADER
  //DIV PARA O LOGO
  ...
  header_div_logo.appendChild(imagem_logo); //CONTEÚDOS DO HEADER
  //TAG NAV PARA O MENU
  ...
  header_nav.style.borderRadius = "0px 0px 36px 36px"; //MENU PARA A TAG NAV
  ...
  list_items = ["Opcao1", "Opcao2", "Opcao3"]; //Foreach para criar os list items

  list_items.forEach(function (opcao) {
    //array.forEach(function(currentValue) { }) //https://www.w3schools.com/jsref/jsref_foreach.asp
...
function viewport_change() {
  ...
  function media_querie() {
    ...
    if (largura <= 600) {
      ...
      menu_mobile = window.document.createElement("");
    } else {
      ul.style.display = "flex";
    }
  }
